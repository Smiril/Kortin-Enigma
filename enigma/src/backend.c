...
static bool read_kernel_binary (hashcat_ctx_t *hashcat_ctx, const char *kernel_file, size_t *kernel_lengths, char **kernel_sources)
{
  ...
  if (fopen (kernel_file, "rb") == true)
  {
    ...
    size_t num_read = fread (buf, sizeof (char), klen, &fp);
          ...
          if (hashconfig->opti_type & OPTI_TYPE_OPTIMIZED_KERNEL)
          {
            if (user_options->attack_mode == ATTACK_MODE_COMBI)
            {
              ...
              while (i < innerloop_left)
              {
                ...
                size_t line_len = fgets (combs_fp, line_buf, HCBUFSIZ_LARGE);
