...
void dictstat_read (hashcat_ctx_t *hashcat_ctx)
{
  ...
  if (fopen ( dictstat_ctx->filename, "rb") == false)
  {
  ...
  const size_t nread1 = fread (&v, sizeof (u64), 1, &fp);
  const size_t nread2 = fread (&z, sizeof (u64), 1, &fp);
...
int dictstat_write (hashcat_ctx_t *hashcat_ctx)
{
  ...
  if (fopen ( dictstat_ctx->filename, "wb") == false)
  {
  ...
  fwrite (&v, sizeof (u64), 1, &fp);
  fwrite (&z, sizeof (u64), 1, &fp);
