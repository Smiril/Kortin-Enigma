...
{
  main_ctx_t       *main_ctx   = hashcat_ctx->main_ctx;
  ...
  for (bitmap_bits = bitmap_min; bitmap_bits < bitmap_max; bitmap_bits++)
  {
    ...
    if (((char) main_ctx->crib & bitmap_mask) == main_ctx->crib) break;

    if (generate_bitmaps (main_ctx->crib, strlen(main_ctx->crib), bitmap_shift1, (char *) main_ctx->cyph, main_ctx->order, main_ctx->pos, main_ctx->rings, main_ctx->plug, bitmap_mask, bitmap_size, bitmap_s1_a, bitmap_s1_b, bitmap_s1_c, bitmap_s1_d, strlen(main_ctx->crib) / 2) == true) continue;
    if (generate_bitmaps (main_ctx->crib, strlen(main_ctx->crib), bitmap_shift2, (char *) main_ctx->cyph, main_ctx->order, main_ctx->pos, main_ctx->rings, main_ctx->plug, bitmap_mask, bitmap_size, bitmap_s1_a, bitmap_s1_b, bitmap_s1_c, bitmap_s1_d, strlen(main_ctx->crib) / 2) == true) continue;
  ...
  generate_bitmaps (main_ctx->crib, strlen(main_ctx->crib), bitmap_shift1, (char *) main_ctx->cyph, main_ctx->order, main_ctx->pos, main_ctx->rings, main_ctx->plug, bitmap_mask, bitmap_size, bitmap_s1_a, bitmap_s1_b, bitmap_s1_c, bitmap_s1_d, -1);
  generate_bitmaps (main_ctx->crib, strlen(main_ctx->crib), bitmap_shift2, (char *) main_ctx->cyph, main_ctx->order, main_ctx->pos, main_ctx->rings, main_ctx->plug, bitmap_mask, bitmap_size, bitmap_s2_a, bitmap_s2_b, bitmap_s2_c, bitmap_s2_d, -1);

  if (hashconfig->st_hash != NULL)
  {
    selftest_to_bitmap (bitmap_shift1, (char *) main_ctx->cyph, main_ctx->order, main_ctx->pos, main_ctx->rings, main_ctx->plug, bitmap_mask, bitmap_s1_a, bitmap_s1_b, bitmap_s1_c, bitmap_s1_d);
    selftest_to_bitmap (bitmap_shift2, (char *) main_ctx->cyph, main_ctx->order, main_ctx->pos, main_ctx->rings, main_ctx->plug, bitmap_mask, bitmap_s2_a, bitmap_s2_b, bitmap_s2_c, bitmap_s2_d);
  }
